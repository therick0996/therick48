...
// Flash bootloader code:
// avrdude -c avrisp -P /dev/cu.usbmodem123451 -p atmega32u4 -U flash:w:bootloader_atmega32u4_1_0_0.hex:i
...
void make_firmware_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (make_firmware_state.state) {
    case SINGLE_TAP: SEND_STRING("make therick48:defaut:dfu"); break; // send therick48 make code
    case DOUBLE_TAP: SEND_STRING("make nori:default:avrdude"); break; // send nori macos make code
  }
}
